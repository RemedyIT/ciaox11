...
foreach $file (@files) {
    ...
    print "Invoking domain deployment manager (dancex11_deployment_manager --handler dancex11_domain_dm_handler)\n";
    ...
    print "Start dancex11_plan_launcher.exe with -l $file -r file://$ior_emfile\n";
    $E = $tg_executor->CreateProcess ("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_plan_launcher",
                            "-l " . $tg_executor->LocalFile($file) . " -r file://$ior_emfile");
    ...
    print "by running dancex11_plan_launcher with -x $file -r file://$ior_emfile\n";

    $E = $tg_executor->CreateProcess ("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_plan_launcher",
                            "-x " . $tg_executor->LocalFile($file) . " -r file://$ior_emfile");
    $pl_status = $E->SpawnWaitKill ($PerlACE::Process::WAIT_DELAY_FACTOR * $tg_executor->ProcessStartWaitInterval ());
