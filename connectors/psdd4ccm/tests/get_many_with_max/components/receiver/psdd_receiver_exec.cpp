...
namespace Test_Receiver_Impl
{
  ...
  Receiver_exec_i::tick ()
  {
    try
    {
      if (this->count () > this->topic_counter_)
      {
        ...
        Test::TopicSeq topics {};
        if (reader->get_many (topics))
        {
          if (topics.size () > 2)
          {
            PSDD4CCM_TEST_ERROR << "Receiver_exec_i::tick : too many topics - read " << topics.size () << std::endl;
          }
          else
          {
            PSDD4CCM_TEST_DEBUG << "Receiver_exec_i::tick : read \"" << IDL::traits<Test::TopicSeq>::write (topics) << "\"" << std::endl;

            this->topic_counter_ += (uint32_t)topics.size ();
            if (topics.size () > 1)
              this->read_multiple_ = true;
          }
        }
        else
        {
  ...
  void Receiver_exec_i::ccm_activate ()
  {
    ...
    if (this->filter ())
    {
      ...
      topic.category (::Test::ECategory::EVEN);
      topic.value (0);
    ...
    // set maximum data for a single get_many to 2
    auto reader = this->context_->get_connection_topic_read ();
    reader->max_data (2);
  ...
  void Receiver_exec_i::ccm_remove ()
  {
    ...
    if (!this->read_multiple_)
      PSDD4CCM_TEST_WARNING << "Receiver_exec_i::ccm_remove : NEVER read multiple topics" << std::endl;
