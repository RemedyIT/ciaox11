...
if (!@files) {
    opendir(DIR, ".");
    @files = grep(/^plan.*\.($plan_fmt)$/,readdir(DIR));
    closedir(DIR);
}
...
sub create_targets {
    ...
    for ($i = 0; $i < $nr_daemon; ++$i) {
        ...
        $tg_daemons[$i]->SetEnv('CIAOX11_TEST_VERBOSE','prio|category|thread|process');
    }
    #   execution manager
...
sub kill_node_daemon {
    for ($i = 0; $i < $nr_daemon; ++$i) {
        ...
        $d_param = "-x -p $port -ORBDottedDecimalAddresses 1 -n $nodename=$iorfile --deployment-nc ". $ENV{'NameServiceIOR'};
