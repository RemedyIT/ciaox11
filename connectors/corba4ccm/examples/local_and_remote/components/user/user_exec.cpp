...
namespace Example_User_Impl
{
      ...
      uint32_t round)
  {
    ...
    if (run_my_bar)
    {
      ...
      run_my_bar->do_something_else (msg);

      CIAOX11_TEST_INFO << this->connection_ << ": Bar returned " << msg << std::endl;
    }
    else
    {
      CIAOX11_TEST_ERROR << "Undefined run_my_bar receptacle!" << std::endl;
    }
  }
