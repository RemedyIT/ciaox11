...
namespace Test_Receiver_Impl
{
  ...
  void Receiver_exec_i::ccm_activate ()
  {
    ...
    if (this->filter ())
    {
      ...
      if (this->partial_key ())
      {
        PSDD4CCM_TEST_DEBUG << "Receiver_exec_i::ccm_activate : subscribing to odd topics" << std::endl;

        // subscribe to odd topic instances
        topic.k ().category (::Test::ECategory::ODD);
      }
      else
      {
        PSDD4CCM_TEST_DEBUG << "Receiver_exec_i::ccm_activate : subscribing to even|two topics" << std::endl;

        // subscribe to even topic instances
        topic.k ().category (::Test::ECategory::EVEN);
        topic.k ().subtype ("Two");
      }
  ...
  bool
  Receiver_exec_i::partial_key ()
  {
    //@@{__RIDL_REGEN_MARKER__} - BEGIN : Test_Receiver_Impl::Receiver_exec_i::partial_key[getter]
    return this->partial_key_;
    //@@{__RIDL_REGEN_MARKER__} - END : Test_Receiver_Impl::Receiver_exec_i::partial_key[getter]
  }
  ...
  Receiver_exec_i::partial_key (
      bool partial_key)
  {
    //@@{__RIDL_REGEN_MARKER__} - BEGIN : Test_Receiver_Impl::Receiver_exec_i::partial_key[setter]
    this->partial_key_ = partial_key;
    //@@{__RIDL_REGEN_MARKER__} - END : Test_Receiver_Impl::Receiver_exec_i::partial_key[setter]
  }
