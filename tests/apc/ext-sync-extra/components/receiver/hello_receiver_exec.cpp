...
namespace Hello_Receiver_Impl
{
      ...
      int32_t& answer)
  {
    ...
    if (this->control_->receipt_count () == 3 || this->control_->receipt_count () == 4)
    {
  ...
  void Receiver_exec_i::ccm_activate ()
  {
    ...
    if (get_a_number() == 10)
    {
      CIAOX11_TEST_INFO << " OK, extra include path is used." << std::endl;
    }
    else
    {
      CIAOX11_TEST_ERROR << " ERROR, extra include path isn't  used." << std::endl;
    }
