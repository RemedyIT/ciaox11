...
#
# @brief   RIDL ExF for DDS4CCM facet loader
#
...
module IDL
  module CCMX11
    module ExF4DDS
      ROOT = 'ridlbe/ccmx11/facets/exf4dds'.freeze
    end
  end
end
...
# base modules
['config'].each do |f|
  require "#{IDL::CCMX11::ExF4DDS::ROOT}/#{f}"
end
...
Dir.glob(File.join(File.dirname(__FILE__), 'visitors', '*.rb')).each do |f|
  require File.join(IDL::CCMX11::ExF4DDS::ROOT, 'visitors', File.basename(f, '.*'))
end

# writers
Dir.glob(File.join(File.dirname(__FILE__), 'writers', '*.rb')).each do |f|
  require File.join(IDL::CCMX11::ExF4DDS::ROOT, 'writers', File.basename(f, '.*'))
end
