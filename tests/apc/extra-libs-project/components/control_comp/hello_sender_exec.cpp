...
namespace Hello_Sender_Impl
{
  ...
  int Sender_exec_i::test_url ()
   {
     CIAOX11_TEST_INFO << "Sender:\t->check extra lib functionality control component" << std::endl;

     ACE::HTTP::URL http_url;

     if (http_url.parse ("http://www.remedy.nl"))
     {
       CIAOX11_TEST_INFO << "Sender:\t->parsed 'http://www.remedy.nl' " << std::endl;
       ...
       if (urlin)
       {
          ...
          if (!sos.bad ())
          {
            ...
            CIAOX11_TEST_ERROR << "ERROR reading buffer in control component" << std::endl;
       }
     }
     else
       CIAOX11_TEST_ERROR << "ERROR parsing url in control component" << std::endl;
     return 0;
   }
