...
namespace Test_Sender_Impl
{
  ...
  Sender_exec_i::tick ()
  {
    try
    {
      ...
      {
        topic.the_key ().category (Test::ECategory::ODD);
        topic.flag(false);
      }
      else
      {
        topic.the_key ().category (Test::ECategory::EVEN);
        topic.flag(true);
      }
      topic.the_key ().value (this->topic_counter_ % 4);
      oss << "Topic #" << this->topic_counter_;
      topic.description (oss.str ());
      topic.opt ('z' - (this->topic_counter_ % 2));
