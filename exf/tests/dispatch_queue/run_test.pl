...
$PROG = $program->CreateProcess ("dispatch_queue", "");
$program_status = $PROG->SpawnWaitKill ($program->ProcessStartWaitInterval());

if ($program_status != 0) {
    print STDERR "ERROR: dispatch_queue returned $program_status\n";
    exit 1;
}
...
if ($exit_status != 0) {
    print STDERR "ERROR: dispatch_queue returned $exit_status\n";
    exit 1;
}
