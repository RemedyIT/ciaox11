...
sub create_interceptor_config {
    copy ($locality_config, './localitymanager.config');
    open (F, ">>./localitymanager.config") || die "Could not open file: $!\n";
    print F "nl.remedy.it.DnCX11.DeploymentInterceptor ciao_test_interceptor create_test_interceptor\n";
    close F;
    for ($i = 0; $i < $nr_daemon; ++$i) {
