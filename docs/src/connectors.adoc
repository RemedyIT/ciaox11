...
In specific cases like the CORBA4CCM connectors the implementation will also allow remote connections (CORBA based
for CORBA4CCM connectors) as a way to connect two _connector fragments_ (or one fragment and an external service). +
...
The creation of any new connector (type), whether the intention is to have a generated connector in the end or not, starts with creating a "statically" and manually defined implementation (which could become the final implementation or a proof of concept).
In case of a connector type with a single, fixed implementation like the TT4CCM connector this way is the only step
 to take but in the case of connector types with variant implementations dependent of user defined IDL types this is just the first step to work out the general design and implementation.
...
The make files generated from this project will generate the required stub and LEM code for the connector, executor
locator and executor code as well as build 3 libraries:

. `efc_connector_impl_stub` for the stub and LEM code;
. `efc_connector_impl_svnt` for the executor locator (servant in original CCM) code and
...
The generated stub and LEM code is standard and will not need any customization of completion. The generated executor
 locator code is also standard but might need customization for special configuration or connection handling constraints/requirements (like is the case with the CORBA4CCM and AMI4CCM connectors).
...
according to the (Lw)CCM standard;
...
The executor locator (which is the *AXCIOMA* version of what used to be the servant in CCM) implements the methods
required for installation of component instances within the *AXCIOMA* framework and is generated in pass:[*_]svnt
.{cpp,h} files (a leftover from the CCM ancestry). +
...
****
IMPORTANT: The generated method signatures of these standard methods should *never* be changed as this is a fixed
requirement of
the framework API.
****
...
****
IMPORTANT: The generated method signature should *never* be changed as this is a fixed requirement of the framework
API. +
...
actually be managed by the component executor instance managed by the executor locator) when setting up the
...
****
NOTE: In principle the rule here to keep in mind is; do not customize any part of the generated code that is not
bracketed by regeneration markers.
****
...
parameters:::
`IDL::traits<Components::SessionContext>::ref_type ctx` reference to the session context instance

+
This method should normally not need to be customized.
...
By default the generated life cycle methods `configuration_complete`, `ccm_activate`, `ccm_deactivate` and
`ccm_remove` will have an empty implementation. +
These methods are main customization points to add connector implementation specific initialization or clean up code.

In addition to these standard life cycle methods the connector executor class will have accessor methods (and member
variables) generated for any attributes (see the example `service_specification` methods) defined as well as factory
methods (and member variables) for facet executors (see the example `get_request_handler` method). +
These methods should normally not need to be customized.

==== Facet executors

Facet executors are local interface implementations of the base stub interfaces generated for the IDL defined
interfaces of the ports that the connector `provides`.

*AXCIOMA* will generate a default implementation for each defined facet of the connector with a corresponding factory
 method in the component executor class.

===== Example facet

The example connector defines a single facet for the local `EFC::FooHandler` interface for which *AXCIOMA* generates an
implementation in the pass:[*_]exec.{h,cpp} files.

====== Factory method

The following factory method is generated in the component executor class.
...
It is possible to customize the constructor for the generated facet executor class in which case it becomes necessary
 to customize the generated implementation for this factory method accordingly.
...
It is allowed to customize everything about this implementation class except for 2 requirements:

. a facet executor implementation *MUST* be provided through the generated factory method and
. the facet executor implementation *MUST* be derived from base stub class for the facet interface (`IDL::traits<
::EFC::CCM_FooHandler>::base_type` in this case).

====== Method descriptions

submit::
The implementation for this method should be customized to provide the intended functionality of submitting requests
for processing.
...
Component executors and facet executors can be extended with user defined methods and member variables (see the
`pass:[[]user_pass:[*]]` named regeneration markers in the generated code) as needed to implement the intended
functionality which can be called (directly or indirectly) from any of the standard methods.

Executor implementations could also define additional (support) classes to provide the implementation either directly
 in the pass:[*_]exec.{h,cpp} files or in separate files (there are sections bracketed with regeneration markers for
 this as well).

== Interaction patterns and templated connectors

It is possible that a newly created connector implementation is recognized as a repeatable interaction pattern which
could be needed for a variation of IDL defined types (such as the standard CORBA4CCM, AMI4CCM, DDS4CCM and PSDD4CCM
connectors). +
In such cases *AXCIOMA* offers a number of options to make repeated implementation of these connectors easier to do.

The next paragraphs prorvide a short introduction of these options but for a more detailed discussion of these
options there is documentation available xref:templated_connectors.adoc[here].

=== IDL templated modules

First of all there is the option of using templated modules in the IDL definition of the connector.

As an example consider the following link:{code_root}/ciaox11/connectors-templated/idl/efc_interface.idl[templated IDL
interface definitions].

.Templated interface definitions for FooConnector
[%collapsible%open]
====
[source,IDL]
----
...
And the link:{code_root}/ciaox11/connectors-templated/idl/efc_data.idl[IDL data definitions].

.Data IDL definitions
[%collapsible%open]
====
[source,IDL]
----
...
For which we create a link:{code_root}/ciaox11/connectors-templated/idl/efc_connector.idl[connector IDL definition] as
follows.
...
Using the IDL templated module definitions in link:{code_root}/ciaox11/connectors-templated/idl/efc_interfac.idl[efc_interface.idl] a FooConnector type for a wide range of Foo request/reply data types can be defined with
minimal effort.

For more details concerning the IDL templated modules feature see the https://www.omg.org/spec/IDL/3.5[OMG IDL 3+]
standard specification.

Most of the component executor code after an initial implementation can simply be copied. Of course the {cxx11}
implementation still needs to be adapted for every set of data types used which will require more or less effort
depending on the degree of similarity of the data types.

=== Templated connector implementation

For the cases where the component executor implementation would follow a fairly constant, predictable pattern
depending on the actual request/reply datatypes *AXCIOMA* offers an additional option for using {cxx} templates for
implementation of the component executor in combination with the IDL templated module approach.

For more details about this option see the xref:templated_connectors.adoc[templated_connectors] document.

== Automated connector generation

For those cases where the repeated component executor implementation would not (completely) follow a predictable
pattern *AXCIOMA* also provides the possibility to extend the RIDL compiler backend with extensions (facets) to
generate all or part of a specific connector implementation.

Going into detail about that option is outside the scope of this documentation but examples can be found for the
standard CORBA4CCM, AMI4CCM, DDS4CCM and PSDD4CCM connectors. The first two using IDL templated modules without {cxx}
templates but instead fully generated connector implementations. The last two using both IDL templated modules as
well as {cxx} templates for the component executor implementation in addtion to generation of type support code to
complement the templated implementation.
