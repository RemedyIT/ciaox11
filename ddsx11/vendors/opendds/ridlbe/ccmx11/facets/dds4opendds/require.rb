...
#
# @brief   DDS4OPENDDS facet loader
#
...
module IDL
  module CCMX11
    module DDS4OPENDDS
      ROOT = 'ridlbe/ccmx11/facets/dds4opendds'
    end
  end
end
...
[ 'config',
  ...
  require "#{IDL::CCMX11::DDS4OPENDDS::ROOT}/#{f}"
end
...
Dir.glob(File.join(File.dirname(__FILE__), 'visitors', '*.rb')).each do |f|
  require File.join(IDL::CCMX11::DDS4OPENDDS::ROOT, 'visitors', File.basename(f, '.*'))
end

# writers
Dir.glob(File.join(File.dirname(__FILE__), 'writers', '*.rb')).each do |f|
  require File.join(IDL::CCMX11::DDS4OPENDDS::ROOT, 'writers', File.basename(f, '.*'))
end
