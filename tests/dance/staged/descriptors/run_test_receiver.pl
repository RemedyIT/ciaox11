...
$ior_nsbase = "ns.ior";
$ior_nsfile = 0;
$ior_receiver_embase = "em_receiver.ior";
$ior_receiver_emfile = 0;
...
$receiver_plan_file = "plan_receiver." . $plan_fmt;
...
sub init_ior_files {
    ...
    $ior_receiver_emfile = $tg_exe_man->LocalFile ($ior_receiver_embase);
    for ($i = 0; $i < $nr_daemon; ++$i) {
...
sub delete_ior_files {
    ...
    $tg_exe_man->DeleteFile ($ior_receiver_embase);
    $tg_exe_man->DeleteFile ($ior_receiver_conn_base);
}
...
# Invoke execution manager to deploy receiver.
print "Invoking domain deployment manager (dancex11_deployment_manager --handler dancex11_domain_dm_handler) with -l $receiver_plan_file\n";
$EM_R = $tg_exe_man->CreateProcess ("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_deployment_manager",
                                  "--handler dancex11_domain_dm_handler -l " . $tg_exe_man->LocalFile($receiver_plan_file) .
                                  " -n ExecutionManager=$ior_receiver_emfile --deployment-nc " .
                                  $ENV{'NameServiceIOR'});
$em_status = $EM_R->Spawn ();
...
# Invoke executor manager - stop the receiver application -.
print "Invoking executor - stop the receiver application -\n";
print "by running dancex11_deployment_manager with -n ExecutionManager=$ior_receiver_emfile -x\n";

$E_R = $tg_exe_man->CreateProcess ("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_deployment_manager",
                        "-n ExecutionManager=$ior_receiver_emfile -x");
$pl_status = $E_R->SpawnWaitKill ($PerlACE::Process::WAIT_DELAY_FACTOR * $tg_exe_man->ProcessStartWaitInterval ());
...
if ($pl_status != 0) {
    ...
    $EM_R->WaitKill ($tg_exe_man->ProcessStopWaitInterval ());
}
