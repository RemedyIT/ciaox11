...
namespace Hello_Receiver_Impl
{
  ...
  void Receiver_exec_i::ccm_passivate ()
  {
    ...
    if (this->nr_of_messages_ != 4)
      {
        CIAOX11_TEST_ERROR << "ERROR number of messages is not 4 but "
                           << this->nr_of_messages_ << std::endl;
      }
  ...
  void Receiver_exec_i::ccm_remove ()
  {
    ...
    CIAOX11_TEST_INFO << "Receiver_exec_i::ccm_remove." << std::endl;
  ...
  //@@{__RIDL_REGEN_MARKER__} - BEGIN : Hello_Receiver_Impl::Receiver_exec_i[get_do_my_foo]
    CIAOX11_TEST_INFO << "Receiver_exec_i::get_do_my_foo." << std::endl;
    if (!this->do_my_foo_)
    {
      this->do_my_foo_ = CORBA::make_reference <do_my_foo_exec_i> (this->context_, this->nr_of_messages_);
    }
