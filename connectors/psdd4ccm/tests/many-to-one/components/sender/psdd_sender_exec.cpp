...
namespace Test_Sender_Impl
{
  ...
  Sender_exec_i::tick ()
  {
    try
    {
      ...
      topic.publisher_id (this->pubid_);
      ...
      topic.value (this->count () - this->topic_counter_);
      oss << "Topic #" << this->topic_counter_;
      topic.description (oss.str ());
      topic.opt ('w');
  ...
  Sender_exec_i::pubid ()
  {
    //@@{__RIDL_REGEN_MARKER__} - BEGIN : Test_Sender_Impl::Sender_exec_i::pubid[getter]
    return this->pubid_;
    //@@{__RIDL_REGEN_MARKER__} - END : Test_Sender_Impl::Sender_exec_i::pubid[getter]
  }
  ...
  Sender_exec_i::pubid (
      uint32_t pubid)
  {
    //@@{__RIDL_REGEN_MARKER__} - BEGIN : Test_Sender_Impl::Sender_exec_i::pubid[setter]
    this->pubid_ = pubid;
    //@@{__RIDL_REGEN_MARKER__} - END : Test_Sender_Impl::Sender_exec_i::pubid[setter]
  }
