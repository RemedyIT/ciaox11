...
namespace Test_Sender_Impl
{
  ...
  Sender_exec_i::tick ()
  {
    try
    {
      ...
      uint32_t batch_num = this->count () / 5;
      while (this->topic_counter_ < this->count () && batch_num-- > 0)
      {
        ::Test::Topic sample {};
        std::ostringstream oss;
        if (++this->topic_counter_ % 2)
        {
          sample.category (Test::ECategory::ODD);
          sample.flag(false);
        }
        else
        {
          sample.category (Test::ECategory::EVEN);
          sample.flag(true);
        }
        sample.value (this->topic_counter_ % 4);
        oss << "Topic #" << this->topic_counter_;
        sample.description (oss.str ());
        sample.opt ('z' - (this->topic_counter_ % 2));
        topics.push_back (std::move (sample));
      }

      writer->write_many (topics);
    }
