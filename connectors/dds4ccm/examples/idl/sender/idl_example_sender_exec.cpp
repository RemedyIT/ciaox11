// -*- C++ -*-
/**
 * @file    idl_example_sender_exec.cpp
 * @author  Marcel Smit
 *
 ...
 */
//@@{__RIDL_REGEN_MARKER__} - HEADER_END : idl_example_sender_impl.cpp[Header]

#include "idl_example_sender_exec.h"
...
namespace Example_BasicPublisher_comp_Impl
{
      ...
      ::DDS::StatusKind status_kind)
  {
    ...
    if (!this->publication_matched_)
    {
      ...
      // start publishing and set publication_matched_ to true, so start_publishing is invoked only once.
  ...
  void
  BasicPublisher_comp_exec_i::check_status ()
  {
    if (!this->publication_matched_)
    {
      ...
      if (writer_dds_entity)
      {
        ...
        {
          DDS4CCM_TEST_DEBUG << "BasicPublisher_comp_exec_i::check_status publication_matched current_count: "
                          << status.current_count() << std::endl;

          if (status.current_count() == 1)
          {
            DDS4CCM_TEST_DEBUG << "BasicPublisher_comp_exec_i::check_status publication_matched received" << std::endl;
            this->publication_matched_ = true;
    ...
    if (!this->CA1DataPub_CSL_)
      {
        this->CA1DataPub_CSL_ =
          CORBA::make_reference <CA1DataPub_CSL_exec_i>
            (this->context_, IDL::traits<Example::CCM_BasicPublisher_comp>::narrow (this->_lock()), this->publication_matched_);
      }
...
//@@{__RIDL_REGEN_MARKER__} - BEGIN : idl_example_sender_impl.cpp[Footer]
