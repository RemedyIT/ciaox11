#---------------------------------------------------------------------
# @file   run_test_extended_base.pl
...
@libpaths = ('../extended/lib','../base/lib');
...
sub create_targets {
    ...
    for ($i = 0; $i < $nr_daemon; ++$i) {
        ...
        $tg_daemons[$i]->AddLibPath ($libpaths[$i]);
    }
...
sub run_node_daemons {
    for ($i = 0; $i < $nr_daemon; ++$i) {
        ...
        if ($tg_daemons[$i]->WaitForFileTimed($iorbase,
            ...
            print STDERR "ERROR: The ior $iorfile file of node daemon $i could not be found\n";
            for (; $i >= 0; --$i) {
                $DEAMONS[$i]->Kill (); $DEAMONS[$i]->TimedWait (1);
            }
            return -1;
