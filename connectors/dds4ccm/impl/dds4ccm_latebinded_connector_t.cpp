...
{
  DDS4CCM_LOG_TRACE (
    "DDS_LateBinded_Connector_T<CCM_TYPE, TOPIC_TYPE, TOPIC_SEQ_TYPE>::topic_name (set)");

  if (this->stop_dds (topic_name))
  {
    DDS4CCM_LOG_DEBUG ("DDS_LateBinded_Connector_T::topic_name - "
      << "Stopping DDS => switching to topic <" << topic_name << ">.");
    ...
    {
      DDS4CCM_LOG_DEBUG ("DDS_LateBinded_Connector_T::topic_name - "
        "DDS is down. Setting topic to <" << topic_name << ">.");

      TopicBaseConnector::topic_name (topic_name);

      DDS4CCM_LOG_DEBUG ("DDS_LateBinded_Connector_T::topic_name - "
        << "Initialize DDS again for topic <" << topic_name << ">.");

      this->do_configuration_complete ();
      this->do_ccm_activate ();

      DDS4CCM_LOG_DEBUG ("DDS_LateBinded_Connector_T::topic_name - "
        << "DDS up and running for topic <" << topic_name << ">.");
    }
    else
    {
      DDS4CCM_LOG_DEBUG ("DDS_LateBinded_Connector_T::topic_name - "
