...
namespace Hello_Receiver_Impl
{
  ...
  void Receiver_exec_i::ccm_remove ()
  {
    ...
    // Three senders invoking hello 5 times each.
    if (this->nr_invocations_ != 15)
    {
      CIAOX11_TEST_ERROR << "ERROR: Receiver_exec_i::ccm_remove - "
        << "Unexpected number of invocations detected. Expected <15> - got <"
        << this->nr_invocations_ << ">." << std::endl;
    }
  ...
  {
    this->do_my_foo_ =
      CORBA::make_reference <do_my_foo_exec_i>
        (this->context_, this->nr_invocations_);
  }
