...
namespace Test_Receiver_Impl
{
  ...
  void Receiver_exec_i::ccm_activate ()
  {
    ...
    {
      Test::Topic topic {};
      topic.key ("sample/type1");

      PSDD4CCM_TEST_DEBUG << "Receiver_exec_i::ccm_activate : subscribing to '" << topic.key () << "' topic instances" << std::endl;
  ...
  void Receiver_exec_i::ccm_remove ()
  {
    ...
    if (this->topic_counter_ ==  this->count ())
      PSDD4CCM_TEST_DEBUG << "Receiver_exec_i::ccm_remove : received " << this->topic_counter_ << " topic instances" << std::endl;
    else
      PSDD4CCM_TEST_ERROR << "Receiver_exec_i::ccm_remove : ONLY " << this->topic_counter_ << " topic instances received" << std::endl;
