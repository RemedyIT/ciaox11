  ...
  module DDS4CCM
    ...
    module DataIDLExtension
      def add_opendds_proj(fidl)
        ...
        if mpc_idl_obj = mpc_file[:ddsx11_opendds_idl_gen]
          ...
          mpc_idl_types_obj = mpc_file[:ddsx11_opendds_types_gen]

          mpc_types_impl_obj = mpc_file[:ddsx11_opendds_types]
        else
          ...
          mpc_stub_obj.base_projects << 'ddsx11_stub'
          mpc_stub_obj.project_dependencies << "#{mpc_id}_ddsx11_opendds_idl_gen"

          mpc_idl_types_obj = MPC::IDLProject.new(:ddsx11_opendds_types_gen, self)
          mpc_idl_types_obj.includes << idl_includes << project.idl_includes
          mpc_file.add_mpc_project(mpc_idl_types_obj)

          mpc_types_impl_obj = MPC::OpenDDSTsStubProject.new(self)
          mpc_file.add_mpc_project(mpc_types_impl_obj)
        end

        # ddsx11_opendds_idl_gen project exist, but idl files is not already part of this project,
        ...
        # add state/event typed entities interfaces generated by :ddsx11_opendds_idl_gen project for idl file
        ...
        prj_dependencies <<
            MPC::CompileDependency.new(:ddsx11_opendds_idl_gen, self) <<
            MPC::LinkDependency.new(:stub, self) <<
            MPC::CompileDependency.new(:ddsx11_opendds_types_gen, self) <<
            MPC::LinkDependency.new(:ddsx11_opendds_types, self,
                                    MPC::LinkDependency.new(:stub, self))
