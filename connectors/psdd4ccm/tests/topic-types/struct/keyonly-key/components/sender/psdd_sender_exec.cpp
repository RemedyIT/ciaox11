  ...
  Sender_exec_i::tick ()
  {
    try
    {
      ...
      if (++this->topic_counter_ % 2)
      {
        ...
        if ((this->topic_counter_ % 4) == 1)
          topic.k ().subtype ("One");
        else
          topic.k ().subtype ("Three");
      }
      else
      {
        topic.k ().category (Test::ECategory::EVEN);
        if (this->topic_counter_ % 4)
          topic.k ().subtype ("Two");
        else
          topic.k ().subtype ("Four");
      }
      topic.number (this->topic_counter_);
      oss << "Topic-" << this->topic_counter_;
      topic.description (oss.str ());
      topic.value (1.123 * this->topic_counter_);
