  ...
  class IDLFileScanner
    ...
    def scan_idl_file_i(idl_file)
      ...
      begin
        ...
        Thread.current[:axp_project] = @recipe.project
        ...
        Thread.current[:axp_project] = nil
      end
...
# IDL::Scanner patch to provide support for JIT generated IDL files
#
module IDL
  class Scanner

    def check_include_with_axpjit(path, fname)
      # first check if IDL include already exists
      unless found = check_include_without_axpjit(path, fname)
        # if not check if this is a JIT-generated IDL file

        # get current AXP project
        project = Thread.current[:axp_project]

        if project
          # check for registered IDLFile
          idlpath = path+fname
          if project.has_idl_file?(idlpath)
            idlfile = project.idl_files[idlpath]
            # check for existence (triggers JIT creation if enabled)
            idlfile.check_existence
            # check for (possibly) created file
            found = File.file?(idlpath) && File.readable?(idlpath)
          end
        end
      end
      ...
      if found
        ...
        project = Thread.current[:axp_project]
        if project
          idlfile = project.idl_files[path+fname]
          if idlfile
    ...
    alias_method_chain :check_include, :axpjit
