...
# locality config
$locality_config="$DANCEX11_ROOT/bin/localitymanager.config";
...
sub create_targets {
    ...
    $tg_client = PerlACE::TestTarget::create_target (2) || die "Create target for client failed\n";
    $tg_client->AddLibPath ('../lib');
    #   execution manager
    $tg_exe_man = PerlACE::TestTarget::create_target (1) || die "Create target for EM failed\n";
}

sub create_lm_config {
    copy ($locality_config, './localitymanager.config');
    open (F, ">>./localitymanager.config") || die "Could not open file: $!\n";
    print F "\nnl.remedy.it.DnCX11.Property nl.remedy.it.DnCX11.LocalityNamingContext \"" . $ENV{'NameServiceIOR'} . "\"\n";
    close F;
    for ($i = 0; $i < $nr_daemon; ++$i) {
        $tg_daemons[$i]->PutFile ('localitymanager.config');
    }
}

sub remove_lm_config {
    unlink ('./localitymanager.config');
    for ($i = 0; $i < $nr_daemon; ++$i) {
        $tg_daemons[$i]->DeleteFile ('localitymanager.config');
    }
}
...
$CL = $tg_client->CreateProcess ("../tester/client", "-k ". $ENV{'NameServiceIOR'});
$status = $CL->SpawnWaitKill ($tg_client->ProcessStartWaitInterval ());
if ($status != 0) {
    print STDERR "ERROR: Testing NamingService registration failed [$status]!\n";
}
...
$CL = $tg_client->CreateProcess ("../tester/client", "-d -k ". $ENV{'NameServiceIOR'});
$status = $CL->SpawnWaitKill ($tg_client->ProcessStartWaitInterval () + 60);
if ($status != 0) {
    print STDERR "ERROR: Testing NamingService deregistration failed [$status]!\n";
}
