...
namespace Hello_Sender_Impl
{
  ...
  //============================================================
    // Worker thread for synchronous invocations for MyB
    //============================================================
    void
    ...
    synch_foo_generator::svc ()
    {
      ...
      CIAOX11_TEST_INFO << "Sender:\t->get_connection_run_my_B " << std::endl;

      IDL::traits<Hello::B>::ref_type my_B =
        this->ciao_context_->get_connection_run_my_B();

      if (!my_B)
      {
        CIAOX11_TEST_ERROR << "ERROR Sender:\t->get_connection_run_my_B "
                          << "returns null" << std::endl;
        return 1;
      }
      Hello::S3 teststruct3;

      Hello::S2 teststruct2;

      Hello::S1 teststruct1;
      teststruct3.s_long (10);
      teststruct2.s_long(teststruct3.s_long());
      teststruct1.s_long(teststruct2.s_long());
      for (uint32_t i = 0; i < 4; ++i)
      {
        int32_t answer = my_B->op_B (i, teststruct1);
        CIAOX11_TEST_INFO << "Sender:\tmy_B->op_B " << answer << std::endl;
        answer = my_B->op_A (i);
        CIAOX11_TEST_INFO << "Sender:\tmy_B->op_A " << answer << std::endl;
      }
