...
$nr_daemon = 1;
@ports = ( 60001, 30001 );
@iorbases = ( "Sender.ior", "Server.ior" );
@nodenames = ( "SenderNode", "ServerNode" );
...
sub create_targets {
    ...
    #   Hello server
    $tg_hello = PerlACE::TestTarget::create_target ($nr_daemon+2) || die "Create target for EM failed\n";
    $tg_hello->AddLibPath ('../lib');
    #   execution manager
...
sub delete_ior_files {
    ...
    $tg_hello->DeleteFile ($iorbases[1]);
...
# Start Hello server
print "Starting Hello server with -p $ports[1] -n HelloServant -o " . $tg_hello->LocalFile ($iorbases[1]) . "\n";
$HS = $tg_hello->CreateProcess ("../server/server", "-p $ports[1] -n HelloServant -o " . $tg_hello->LocalFile ($iorbases[1]) .
                                                    " -ORBInitRef NameService=" . $ENV{"NameServiceIOR"});

$hs_status = $HS->Spawn ();

if ($hs_status != 0) {
    print STDERR "ERROR: Hello server returned $hs_status\n";
    kill_open_processes ();
    exit 1;
}

if ($tg_hello->WaitForFileTimed ($iorbases[1],
                                 $tg_hello->ProcessStartWaitInterval ()) == -1) {
    print STDERR "ERROR: The ior file of Hello server could not be found\n";
    kill_open_processes ();
    exit 1;
}
